...
# ╔═╡ a8c3c330-857e-11ef-0186-5bfd830a2dda
using Lux,Random,DifferentiationInterface,Optimisers,PlutoUI

# ╔═╡ 39dc2709-f486-4302-aa07-8fdef5fa1c2a
using Plots

# ╔═╡ a479a319-4d88-45b2-ab01-2094e01964a5
begin
	using CUDA,LuxCUDA,cuDNN;
	CUDA.allowscalar(false)
	
	const gdev = gpu_device()
end

# ╔═╡ c5d53dab-450a-4321-b6f5-b63dc178af85
using OrdinaryDiffEq

# ╔═╡ 30919ee2-b80c-431d-bdc2-084f7ade5a74
using ComponentArrays,SciMLSensitivity

# ╔═╡ 2f80af3f-ca63-44a5-b8a7-371225f9a245
import Flux,Enzyme,Zygote,Tracker,FiniteDiff

# ╔═╡ 4a0e8889-5b9a-4f3e-8a3d-6fee90a9ac45
PlutoUI.TableOfContents()

# ╔═╡ 50e8fc34-e9e4-4205-b163-1174fb5bd86f
begin
	const bckZ = AutoZygote()
	const bckE = AutoEnzyme()
	const bckT = AutoTracker()
	const bckF = AutoFiniteDiff()
end

# ╔═╡ 16961e40-2773-4ef4-ad11-c9ec25b18136
md"# MLP"

# ╔═╡ 9be0d35e-edf7-4316-92cf-7a2d893788e4
begin
	x = rand(1,1000).*2 .- 1
	f(x) = x^2
	y = f.(x)
...
# ╔═╡ f0f75bee-0cf5-4748-8fd0-6e5dbfaa1c9c
scatter(x',y')

# ╔═╡ dbe3542c-e895-41c7-8efe-e0313705e5ca
begin
...
# ╔═╡ 9dc3cf82-4e54-44d4-8e10-af843c55dea5
function loss(model,x,ps,st,y)
	y_pred,_ = Lux.apply(model,x,ps,st)
	Flux.mse(y_pred,y)
end

# ╔═╡ ab4a1464-a55b-473a-adb0-b23c6378be3b
begin
	struct MyDescent{T<:Number} <: AbstractRule
		lr ::T
	end
	MyDescent(;lr) = MyDescent(lr)
	Optimisers.init(o::MyDescent, x::AbstractArray) = nothing
	function Optimisers.apply!(o::MyDescent, state, x, dx)
	  return state, dx * o.lr
	end
end

# ╔═╡ 93508fd6-742a-4e48-ac35-9ca58be22f52
begin
	lr = 0.01
	rule = MyDescent(lr)
...
# ╔═╡ 6fd37d15-14c7-4b0e-a1dd-89021d7e18f7
function train_model!(model,x,ps,st,y,opt;epoch = 16)
	data = Flux.DataLoader((x,y),batchsize = 6,shuffle = true)
	for i in 1:epoch
		l = loss(model,x,ps,st,y)
		@info "i = $i, L = $l"
		for (d1,d2) in data
			g = gradient(ps->loss(model,d1,ps,st,d2),bckZ,ps)
			Optimisers.update!(opt,ps,g)
		end
	end
	ps,opt
end

# ╔═╡ b749ade8-b45e-4ba8-97e6-4389467fc31e
@time train_model!(model,x,ps,st,y,opt)

# ╔═╡ 92485a90-9583-4742-8d8a-562e450459af
y_pred = Lux.apply(model,x,ps,st)[1]

# ╔═╡ 845ab080-ce5b-4e4a-be5f-6458acd0f84a
begin
	scatter(x',y_pred')
	scatter!(x',y')
end

# ╔═╡ 4f391006-3e2f-40d7-9a4b-932ce8df2ea6
md"# ODE PINN"

# ╔═╡ 3c90b1ac-ec75-4134-b957-6db29d22869a
begin
	t = rand(1,1000)
	u0() = 1.0
	f(u,t) = -t*u # u'(t) = -u
...
# ╔═╡ 12c5125f-0e89-4a1c-add4-f78d3f234d3f
begin
	rngODE = Random.default_rng()
	Random.seed!(rngODE, 1)
	sig(x) = 1/(1+exp(-x))
	modelODE = Lux.Chain(
		Lux.Dense(1,16,sig),
		Lux.Dense(16,16,sig),
	...
	psODE,stODE = Lux.setup(rngODE,modelODE)
	psODE = f64(psODE)
end

# ╔═╡ 3da62a8e-4649-4add-a7bc-373f6c62687b
function PINNODE(u_net,t,F)
	dudt = gradient(sum ∘ u_net,bckZ,t)
	return sum(abs2,dudt .- F)
end

# ╔═╡ 85b92e37-6158-4c99-90c3-4496ba82f0d0
der_t = gradient(t->sum(first(modelODE(t,psODE,stODE))),bckZ,t)

# ╔═╡ 46f88fc8-2934-479d-b661-dcc1f58d67b4
begin
	lrODE = 0.1
	ruleODE = Adam(lr)
	optODE = Optimisers.setup(ruleODE,psODE)
end

# ╔═╡ 2fce1e21-5327-4cf1-bcaf-87ac4c6c01a0
y_predODE,_ = modelODE(t,psODE,stODE)

# ╔═╡ 20c4cbe2-7e7b-441a-a1cc-9a9676539aad
begin
scatter(t',y_predODE')
	scatter!(t',uex.(t'))
end

# ╔═╡ 7c7efbf6-ec51-4f57-9763-3fea9739e17d
md"# PDE PINN"

# ╔═╡ e1e216ea-a912-457e-b529-4b3b60232e52
begin
	xxx = range(0.0,1.0,30) 
	ttt = range(0.0,1.0,30) 
	xtp = Iterators.product(ttt,xxx) |> collect |> x-> x[:]
	u0(x) = exp(-50*(x-0.5)^2)
	xt = [ t[k] for k in 1:2, t in xtp] |> gdev
end

# ╔═╡ 4070761d-d214-4774-a8b8-be65454bd9c5
function lossODE(model,t,ps,st,der_t)
	u_net = Lux.StatefulLuxLayer{true}(model,ps,st)
	y_pred,_ = Lux.apply(model,t,ps,st)
	y0,_ = Lux.apply(model,zero(t),ps,st)
	inc = sum((y0 .- u0()).^2)*2/length(t)
	ode = PINNODE(u_net,t, f.(y_pred,t)) 
	0.5*inc + 0.5*ode
end

# ╔═╡ 51c78f47-0b0a-4b1a-b839-73ac9bc25616
lossODE(modelODE,t,psODE,stODE,der_t)

# ╔═╡ 623d8c5e-062a-4f29-b56f-c38259dc9039
 g=gradient(ps->lossODE(modelODE,t,ps,stODE,der_t),bckT,psODE)

# ╔═╡ db60a2cc-b1ef-45c9-be1d-6da9172423f0
Optimisers.update!(optODE,psODE,g)

# ╔═╡ d6a9f9c9-40cf-45df-9c2a-54d2cb9f3dd6
function train_modelODE!(model,t,ps,st,opt,der_t;epoch = 16)
	for i in 1:epoch
		l = lossODE(model,t,ps,st,der_t)
		@info "i = $i, L = $l"
		g = gradient(ps->lossODE(model,t,ps,st,der_t),bckZ,ps)
		Optimisers.update!(opt,ps,g)
	end
	ps,opt
end

# ╔═╡ d84f7063-5f3d-4c8e-82c0-3884014e7792
@time train_modelODE!(modelODE,t,psODE,stODE,optODE,der_t;epoch = 100)

# ╔═╡ 5ff6ffcd-96d9-4a0e-ae03-50e0e7c6a357
begin
	rngPDE = Random.default_rng()
	Random.seed!(rngPDE, 3)
	modelPDE = Lux.Chain(
		Lux.Dense(2,64,Lux.relu),
		Lux.Dense(64,64,Lux.relu),
		Lux.Dense(64,64,Lux.relu),
		Lux.Dense(64,1)
	)
	psPDE,stPDE = Lux.setup(rngPDE,modelPDE) |> gdev
	# psPDE = f64(psPDE)
end

# ╔═╡ 8cd6dba2-5fb2-4894-b4da-9d259a7eee0f
function PDENN(u_net,u_pred,xt)
	dudxt = gradient(sum ∘ u_net, bckZ,xt)
	dudt = dudxt[1:1,:]
	dudx = dudxt[2:2,:]
	return sum(abs2,dudt.+u_pred.*dudx)./size(dudt,2)
end

# ╔═╡ f4bffde6-c803-4d83-ae59-971332a3fbba
function lossPDE(model,xt,ps,st,der_xt)
	u_net = StatefulLuxLayer{true}(model, ps, st)
	u_pred,_ = Lux.apply(model,xt,ps,st)
	y0,_ = Lux.apply(model,xt.*cu([0,1]),ps,st)
	yx0,_ = Lux.apply(model,xt.*cu([1,0]),ps,st)
	inc = sum((y0 .- u0.(xt[2:2,:])).^2)*2/length(y0)
	incx = sum(yx0.^2)*2/length(y0)
	pde = PDENN(u_net,u_pred,xt)
	0.25*inc + 0.5*pde+0.25*incx
end

# ╔═╡ 9b6418b5-986b-412c-a37c-cfede2daf94a
der_xt = gradient(xt->sum(first(modelPDE(xt,psPDE,stPDE))),bckZ,xt)

# ╔═╡ e9e50392-9a6d-4bf3-b7ee-bc1a349ca345
lossPDE(modelPDE,xt,psPDE,stPDE,der_xt)

# ╔═╡ f9ce3cf4-1fbc-4cb6-be5e-fe70a9a7c199

function train_modelPDE!(model,xt,ps,st,opt,der_xt,lr;epoch = 16)
	dtf = Flux.DataLoader(xt,batchsize = 32,shuffle = true) |> gdev
	lrs = lr
	Optimisers.adjust!(opt,lrs)
	for i in 1:epoch
		l = lossPDE(model,xt,ps,st,der_xt)
		@info "i = $i, L = $l"
		for d1 in dtf
			g = gradient(ps->lossPDE(model,d1,ps,st,der_xt),bckZ,ps)
			Optimisers.update!(opt,ps,g)
		end
		if i %20 == 0 && lrs >1e-6
			lrs /=1.5 
...
# ╔═╡ 53b90677-a2b9-453c-92e3-d01c527f8046
begin
	lrPDE = 5e-4
	rulePDE = Adam(lr)
	optPDE = Optimisers.setup(rulePDE,psPDE)
end

# ╔═╡ 1bcc021b-adff-4da7-bdf9-f4bd64e36cd4
import TidierData,DataFrames

# ╔═╡ 8ee2227f-c3be-41d2-bcb4-9e643d6080b1
begin
	@time train_modelPDE!(modelPDE,xt,psPDE,stPDE,optPDE,der_xt,lrPDE;epoch = 100)
	xx = range(0.0,1.0,100)
	yy = range(0.0,1.0,100)
	xtppp = Iterators.product(xx,yy) |> collect |> x-> x[:]
	xtpp = [i[k] for k in 1:2, i in xtppp ] |> gdev
	modd = Lux.testmode(StatefulLuxLayer{true}(modelPDE,psPDE,stPDE))
	val = modd(xtpp)
	xx,yy,zz=collect.((xtpp[1,:],xtpp[2,:],val[1,:]))
	df = DataFrames.DataFrame(:t => xx, :x => yy, :u => zz)
	res = TidierData.@group_by(df,t)
	nothing
end

# ╔═╡ fe34a91b-191b-45ea-a2d2-1bf1913c1990
@gif for dfi in res
	plot(dfi.x,dfi.u)
end

# ╔═╡ 732817b4-31f4-42f7-9626-a6eb991ef71d
md"# Neural ODE"

# ╔═╡ fd8a0f5e-14fb-4c62-b2f6-03169794da6f
md"## Create false datas"

# ╔═╡ 44d53fb6-2f14-4022-9b89-9f575d276a23
function true_ode(u,p,t)
	-u # u'(t) = -u(t)
end

# ╔═╡ 64e99313-9193-47b4-b65e-e8d0cc9df231
begin
	const dt = 0.01
	const tspan = (0.0,1.0)
	const u00 = [2.0]
...
# ╔═╡ 7dfe52e7-86d6-48c4-b7e0-779845459839
prob = ODEProblem(true_ode,u00,tspan,pODE)

# ╔═╡ 70df0386-2ca4-4628-96c8-97218d5223c7
sol = solve(prob,Tsit5(),dt = 0.01,saveat=0.01)

# ╔═╡ 53bc33db-d430-4d6a-ab91-f0329058972e
Array(sol)

# ╔═╡ 8c94e1d2-1591-4715-a808-d1b656496568
plot(sol.t,Array(sol)')

# ╔═╡ 26d7f0d8-eb25-45db-98e1-c11f974683dd
md"## Find back evolution"

# ╔═╡ 5b49f487-27e9-4142-8f50-9d2f641e5caa
begin
	const nnODE = Chain(
		Dense(1=>16,relu),
		Dense(16=>16,relu),
	...
	nnps, nnst = Lux.setup(Random.default_rng(), nnODE) |> f64 
end

# ╔═╡ f1ef3a6e-b9ba-4b0e-afaa-1eb8242d964d
params = ComponentArray(nnps)

# ╔═╡ 938fdc9a-2454-4bfe-a6d9-9d6c4ba79cbc
const nnODE_model = StatefulLuxLayer{true}(nnODE, nothing, nnst)

# ╔═╡ 804a8cca-905f-4d1e-b697-c22deb1c970c
nnODE_model([1.0],params)

# ╔═╡ 663414b5-3c7d-4d39-a79e-468f8339e944
function learn_ode(u,nn_param,t)
	return nnODE_model(u,nn_param)
end

# ╔═╡ e2f8caf1-1835-44b6-994b-5360fdd7fb56
probnn = ODEProblem(learn_ode,u00,tspan,params)

# ╔═╡ b3af787b-4aa2-4f75-b26d-661a1b0bdbbe
solnn = solve(probnn,Tsit5(),dt = dt,saveat = 0.01,adaptive = false)

# ╔═╡ 9df7777f-0910-4888-8de9-362c93e7ef9b
plot(solnn.t,Array(solnn)')

# ╔═╡ aca658f0-5ab6-4574-9e2a-541fa92e3ceb
md"## Let's train it !"

# ╔═╡ 388a11bc-bf5b-4517-8c61-5f1a8f381338
function lossnn(theta,true_sol)
	probnn = ODEProblem(learn_ode,u00,tspan,theta)
    pred = Array(solve(probnn, Tsit5(); saveat=0.01, dt = 0.01, adaptive=false))
    return sum(abs2,pred.-true_sol)
end

# ╔═╡ afc1bb19-c906-4134-b0ca-7c63c07ea49d
lossnn(params,Array(sol))

# ╔═╡ b8f2faa8-85c1-4236-84db-747e4226d54c
gradient(params -> lossnn(params,Array(sol)),bckZ,params)

# ╔═╡ 10412a01-4016-4702-98c7-1aa6f7d1059f
begin
	optnn = Optimisers.setup(Adam(0.1),params)
end

# ╔═╡ 120877e0-679c-448d-99be-77c87edbe9bd
for i in 1:32
	gin = gradient(params -> lossnn(params,Array(sol)),bckZ,params)
	Optimisers.update!(optnn,params,gin)
	@info lossnn(params,Array(sol))
end

# ╔═╡ 6074b5dd-5ea9-4c1f-81bb-9663fc04fa02
solnn_t = solve(probnn,Tsit5(),dt = dt,saveat = 0.01,adaptive = false)

# ╔═╡ 367f75bf-52a7-4ceb-b492-47391f1ff834
begin
	plot(solnn_t.t,Array(solnn_t)')
	plot!(solnn_t.t,t->u00[1]*exp.(-t))
end

# ╔═╡ ed3f388c-afbe-499d-a3ca-32da1a17dcc8
u02 = [3]

# ╔═╡ 080fbca2-f466-41ff-997b-7ed64d14e2df
probnn2 = ODEProblem(learn_ode,u02,tspan,params)

# ╔═╡ 1100da66-b475-42d8-86de-2f26083942ad
solnn_t2 = solve(probnn2,Tsit5(),dt = dt,saveat = 0.01,adaptive = false)

# ╔═╡ 4d3f2cc8-d30d-44a2-bc6a-e3f067205bbc
begin
	plot(solnn_t2.t,Array(solnn_t2)')
	plot!(solnn_t2.t,t->u02[1]*exp.(-t))
end
...
DataFrames = "~1.7.0"
DifferentiationInterface = "~0.6.7"
Enzyme = "~0.13.7"
FiniteDiff = "~2.26.0"
Flux = "~0.14.21"
Lux = "~1.1.0"
...
manifest_format = "2.0"
project_hash = "d8149ec40ce8a774d8270ada2977bd1263556ffe"
...
git-tree-sha1 = "8977ef8249b602e4cb46ddbaf3c51e6adc2958c7"
...
git-tree-sha1 = "a8a5906be27c42ea0705a2daed04fd57fd12d055"
...
version = "1.0.1"
weakdeps = ["ArrayInterface", "ChainRulesCore", "EnzymeCore", "Functors", "MLDataDevices", "ReverseDiff", "Setfield", "Tracker"]
...
git-tree-sha1 = "d01eebc2dbd30c83a857ae8fcc7f12ea6bd5b10c"
...
deps = ["ADTypes", "Accessors", "Adapt", "ArrayInterface", "ChainRulesCore", "DiffEqBase", "DiffEqCallbacks", "DiffEqNoiseProcess", "Distributions", "EllipsisNotation", "Enzyme", "FiniteDiff", "ForwardDiff", "FunctionProperties", "FunctionWrappersWrappers", "Functors", "GPUArraysCore", "LinearAlgebra", "LinearSolve", "Markdown", "Parameters", "PreallocationTools", "QuadGK", "Random", "RandomNumbers", "RecursiveArrayTools", "Reexport", "ReverseDiff", "SciMLBase", "SciMLOperators", "SciMLStructures", "SparseDiffTools", "StaticArrays", "StaticArraysCore", "Statistics", "SymbolicIndexingInterface", "Tracker", "TruncatedStacktraces", "Zygote"]
git-tree-sha1 = "b669eb03c0d5a92ab6a337549afcaad2c9b381e0"
